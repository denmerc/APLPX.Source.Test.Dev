    ...
    public class SqlService {
            ...
            if (sqlConnection.State == ConnectionState.Open) {
                try {
                    ...
                    if (sqlAdapter.Fill(sqlDataTable) == 0) {
                        sqlMessage = "APLPXServices.sqlService.ExecuteReader request returned zero records.";
                    }
                    ...
                    sqlMessage = "APLPXServices.sqlService.ExecuteReader, Invalid data adapter, " + ex1.Source + ", " + ex1.Message;
                }
                catch (System.InvalidOperationException ex2) {
                    sqlMessage = "APLPXServices.sqlService.ExecuteReader, Invalid data table, " + ex2.Source + ", " + ex2.Message;
                }
                catch (Exception ex3) {
                    sqlMessage = "APLPXServices.sqlService.ExecuteReader, " + ex3.Source + ", " + ex3.Message;
                }
            }
            //return sqlDataTable;
            return sqlDataTable;
            ...
            if (sqlConnection.State == ConnectionState.Open) {
                try {
                    ...
                    if (sqlAdapter.Fill(sqlDataSet) == 0) {
                        sqlMessage = "APLPXServices.sqlService.ExecuteReaders request returned zero tables.";
                    }
                    for (int i = 0; i < this.sqlParameters.List.Length; i++) {
                        if (this.sqlParameters.List[i].dbDirection == ParameterDirection.InputOutput || this.sqlParameters.List[i].dbDirection == ParameterDirection.Output) {
                            this.sqlParameters.List[i].dbOutput = sqlAdapter.SelectCommand.Parameters[this.sqlParameters.List[i].dbName].Value.ToString();
                    ...
                    sqlMessage = "APLPXServices.sqlService.ExecuteReaders, Invalid data adapter, " + ex1.Source + ", " + ex1.Message;
                }
                catch (System.InvalidOperationException ex2) {
                    sqlMessage = "APLPXServices.sqlService.ExecuteReaders, Invalid data tables, " + ex2.Source + ", " + ex2.Message;
                }
                catch (Exception ex3) {
                    sqlMessage = "APLPXServices.sqlService.ExecuteReaders, " + ex3.Source + ", " + ex3.Message;
                }
            }
            return sqlDataSet;
        }
        ...
        public Boolean ExecuteNonQuery() {
            ...
            if (sqlConnection.State == ConnectionState.Open) {
                try {
                    ...
                    sqlMessage = "APLPXServices.sqlService.executeNonQuery, Invalid command, " + ex1.Source + ", " + ex1.Message;
                }
                catch (Exception ex2) {
                    sqlMessage = "APLPXServices.sqlService.executeNonQuery, " + ex2.Source + ", " + ex2.Message;
                }
            }
            return sqlExecuted;
        }
        ...
        public Boolean ExecuteCloseConnection() {
            ...
            try {
                ...
                sqlMessage = "APLPXServices.sqlService.executeCloseConnection, " + ex2.Source + ", " + ex2.Message;
            }
